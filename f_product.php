...
if (!empty($search)) {
    $where[] = "f_product.name LIKE ?";
    $params[] = "%" . $search . "%";
    $types .= 's';
}

if (is_numeric($price_min)) {
    $where[] = "f_product.price >= ?";
    $params[] = $price_min;
    $types .= 'd';
}
...
if (!empty($brands)) {
    ...
    $where[] = "f_product.brand IN ($placeholders)";
...
if (!empty($screen_sizes)) {
    ...
    $where[] = "f_product.screen_size IN ($placeholders)";
...
if (!empty($category_id)) {
    ...
    $where[] = "f_product.category_id IN ($placeholders)";
    $params = array_merge($params, $category_id);
    $types .= str_repeat('i', count($category_id));
}

if (is_numeric($hdmi)) {
    $where[] = "f_product.hdmi_ports >= ?";
    $params[] = $hdmi;
    $types .= 'i';
}

if (is_numeric($usb)) {
    $where[] = "f_product.usb_ports >= ?";
    $params[] = $usb;
    $types .= 'i';
}

$sql = "SELECT f_product.id, f_product.name, f_product.price, f_product.image_url, f_category.name AS fcategory_name
        FROM f_product
        JOIN f_category ON f_product.category_id = f_category.id";

if (!empty($where)) $sql .= " WHERE " . implode(" AND ", $where);
$sql .= " ORDER BY f_product.id DESC";
...
$categoriesQuery = $conn->query("SELECT id, name FROM f_category ORDER BY name");
$brandsQuery = $conn->query("SELECT DISTINCT brand FROM f_product ORDER BY brand");
$sizesQuery = $conn->query("SELECT DISTINCT screen_size FROM f_product ORDER BY screen_size");
?>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <?php if ($result->num_rows > 0): while ($row = $result->fetch_assoc()): ?>
          <div class="bg-white rounded shadow p-4 hover:shadow-lg transition">
            ...
            <p class="text-sm text-gray-500 mb-1">Category: <?= htmlspecialchars($row['fcategory_name']) ?></p>
            <p class="text-blue-600 font-bold">₹<?= number_format($row['price'], 2) ?></p>
            <a href="f_product_detail.php?id=<?= encrypt_id($row['id']) ?>" class="mt-2 block text-center bg-blue-500 text-white py-1 rounded hover:bg-blue-600 transition">
              View Details
            </a>
          </div>
        <?php endwhile; else: ?>
