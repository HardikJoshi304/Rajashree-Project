...
$category_id_decrypt = isset($_GET['id']) ? $_GET['id'] : 0;
$category_id = decrypt_id($category_id_decrypt);

session_start();
if (!is_numeric($category_id)) {
  $_SESSION['toast_message'] = "Invalid ID!";
...
$brandsQuery = $conn->query("SELECT DISTINCT brand FROM f_product WHERE category_id = $category_id ORDER BY brand");
$sizesQuery = $conn->query("SELECT DISTINCT screen_size FROM f_product WHERE category_id = $category_id ORDER BY screen_size");
?>
